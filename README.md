# Fetch.ai x Internet Computer (Rust Backend)

This repository is a Rust-based remake of the [Fetch.ai + ICP Integration](https://github.com/fetchai/innovation-lab-examples/tree/main/web3/internet-computer), which originally used Azle for the backend. This version replaces the Azle backend with a Rust-based implementation.
...
└── ic/                       # Internet Computer backend (Rust canister)
    ├── src/
    │   └── backend/          # Rust canister: backend
    │       ├── src/          # Rust source files for the canister
    │       │   └── lib.rs    # Canister entry point
    │       ├── backend.did   # Candid interface definition
    │       └── Cargo.toml    # Rust package config
    ├── Cargo.lock            # Dependency lockfile (autogenerated)
...
The ICP component (`ic/src/backend/src/lib.rs`) implements a dummy HTTP server with the following endpoints:
...
5. **In the browser, open and interact with HTTP Server:**
   - URL: http://{canister_backend_id}.raw.localhost:4943/ (see id from deploy message)
